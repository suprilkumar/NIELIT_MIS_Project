...
import {
    FaPlus, FaSearch, FaFilter, FaTimes, FaDownload,
    FaEdit, FaEye, FaTrash, FaChevronLeft, FaChevronRight,
    FaBuilding, FaBook, FaCalendarAlt, FaCheck, FaLock,
    FaExclamationTriangle, FaFilePdf, FaFileCsv, FaPrint,
    FaChartBar, FaSync, FaClock, FaChartLine,
    FaGraduationCap, FaUsers, FaCheckCircle, FaHourglassHalf
} from 'react-icons/fa';
...
export default function AdminCourseEntryListPage() {
    ...
    const fetchEntries = async () => {
        setLoading(true);
        try {
            const response = await api.get('/course-entry/list/');
    ...
    const handleDelete = async (id, centreName, courseName) => {
        if (window.confirm(`Are you sure you want to delete the entry for ${centreName} - ${courseName}? This action cannot be undone.`)) {
            try {
                await api.delete(`/course-entry/manage/${id}/`);
    ...
    const exportToPDF = () => {
        ...
        try {
            ...
            // Title
            doc.setFontSize(18);
            doc.text('Course Entry Report', 14, 22);

            // Date range
            doc.setFontSize(10);
            let reportDate = `Generated on: ${new Date().toLocaleDateString()}`;
            if (filters.fromDate && filters.toDate) {
                reportDate += ` | Period: ${filters.fromDate} to ${filters.toDate}`;
            } else if (filters.month && filters.year) {
                const monthName = months.find(m => m.value === filters.month)?.label;
                reportDate += ` | Month: ${monthName} ${filters.year}`;
            }
            ...
            doc.setFontSize(10);
            doc.text(`Total Entries: ${summary.totalEntries}`, 14, 48);
            doc.text(`Total Enrolled: ${summary.totalEnrolled.toLocaleString()}`, 14, 55);
            doc.text(`Total Trained: ${summary.totalTrained.toLocaleString()}`, 14, 62);
            doc.text(`Total Certified: ${summary.totalCertified.toLocaleString()}`, 14, 69);
            doc.text(`Total Placed: ${summary.totalPlaced.toLocaleString()}`, 14, 76);
            doc.text(`Pending: ${summary.pendingCount}`, 80, 48);
            doc.text(`Verified: ${summary.verifiedCount}`, 80, 55);
            ...
            const tableRows = filteredEntries.map((entry, index) => [
                index + 1,
                entry.centre_name || 'N/A',
                entry.course_name || 'N/A',
                entry.month_year ? new Date(entry.month_year).toLocaleDateString('en-IN', { year: 'numeric', month: 'short' }) : 'N/A',
                entry.total_enrolled || 0,
                entry.total_trained || 0,
                entry.total_certified || 0,
                entry.total_placed || 0,
                entry.entry_status || 'N/A'
            ]);
            ...
            doc.save(`course_entries_${new Date().toISOString().split('T')[0]}.pdf`);
    ...
    return (
        <>
            <Head>
                <title>Course Entries | Admin</title>
            </Head>
                ...
                <div className="max-w-7xl mx-auto">
                    ...
                    <div className="mb-8">
                        <div className="flex items-center justify-between">
                            ...
                            {/* Add Entry Button */}
                            <Link
                                href="/dashboard/admin/course-entry/create"
                                ...
                                <div className="relative group">
                                    ...
                                    <div className="absolute right-0 mt-2 w-56 bg-white rounded-lg shadow-lg border border-gray-200 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-10">
                                        <button
                                            ...
                                            className="w-full flex items-center gap-3 px-4 py-3 text-sm text-gray-700 hover:bg-gray-50 disabled:opacity-50 first:rounded-t-lg"
                                        >
                                    ...
                                    {paginatedEntries.length > 0 ? (
                                        paginatedEntries.map((entry, index) => (
                                            <tr key={entry.id} className="hover:bg-gray-50 transition-colors">
                                                ...
                                                <td className="px-6 py-4 text-sm text-gray-600">
                                                    {entry.created_by_username || 'N/A'}
                                                </td>
                                                <td className="px-6 py-4">
                                                    <div className="flex items-center gap-2">
                                                        <Link
                                                            href={`/dashboard/admin/course-entry/view/${entry.id}`}
